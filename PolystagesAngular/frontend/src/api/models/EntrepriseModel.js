...
Entreprise.getEntrepriseById = function (identreprise, result) {

  const query = `Select * from entreprise where identreprise = ${identreprise} `;
  db.query(query, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
...
Entreprise.getEntrepriseByName = function (entrepriseName, result) {
  const query = `Select * from entreprise where nomcomplet = "${entrepriseName}" `;
  db.query(query, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
...
Entreprise.createEntreprise = function (bodyForm, result) {
  const new_entreprise = new Entreprise(bodyForm);
