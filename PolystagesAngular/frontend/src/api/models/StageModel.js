...
Stage.getStageById = function (idstage, result) {
  const query = "Select * from stage left join tuteurs on tuteurs.idtuteur = stage.idtuteur left join entreprise on entreprise.identreprise = stage.identreprise where idstage = ?";
...
Stage.getStageByIdForEval = function (idstage, result) {
  const query = "Select * from stage natural join eleves left join entreprise on entreprise.identreprise = stage.identreprise where idstage = ?";
  db.query(query, idstage, function (err, res) {
        for (i in res) {
            res[i] = DateUpdate(res[i])
        }
...
Stage.getStageByEleveId = function (ideleve, result) {
  const query = "Select * from stage natural join entreprise where ideleve = ? order by annee desc";
...
Stage.getStageByTuteurId = function (idtuteur, result) {
  const query = "Select * from stage natural join entreprise left join eleves on eleves.ideleve = stage.ideleve where idtuteur = ? order by annee desc";
...
Stage.getStageByEnsId = function (idens, result) {
  const query = "Select * from stage natural join entreprise left join eleves on eleves.ideleve = stage.ideleve where idens = ? order by annee desc";
...
Stage.getCurrentStageByEleveId = function (annee, ideleve, result) {
  ...
  const query = "Select * from stage where ideleve = ? and annee = ?";
...
Stage.getCurrentStageByTuteurId = function (annee, idtuteur, result) {
  ...
  const query = "Select * from stage left join eleves on eleves.ideleve = stage.ideleve where idtuteur = ? and annee = ?";
...
Stage.getCurrentStageByEnsId = function (annee, idens, result) {
  ...
  const query = "Select * from stage left join eleves on eleves.ideleve = stage.ideleve where idens = ? and annee = ?";
...
Stage.getStageByAnnee = function (result) {
  const query = "SELECT annee, COUNT(annee) as nbre_stage FROM stage GROUP BY annee";
  db.query(query, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
...
Stage.getStageByLevel = function (result) {
  const query = "SELECT niveau, COUNT(niveau) as nbre FROM stage GROUP BY niveau";
  db.query(query, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
...
Stage.getStageByCity = function (result) {
  const query = "SELECT ville, COUNT(ville) as nbre_stage FROM `stage` GROUP BY ville";
  db.query(query, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
