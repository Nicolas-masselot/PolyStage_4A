...
Tuteur.getAllTuteur = function (result) {
  ...
  const query = "Select * from tuteurs";
...
Tuteur.getTuteurById = function (idtuteur, result) {
  const query = "Select * from tuteurs where idtuteur = ?";
...
Tuteur.getTuteurIdByNomAndPrenom = function (nom, prenom, result) {
  const query = ` Select * from tuteurs where (nom = "${nom}") and (prenom = "${prenom}" ) `;
  db.query(query, function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
